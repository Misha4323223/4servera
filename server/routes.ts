...
*🚀 Анализ выполнен с помощью бесплатных AI сервисов!*`;
  ...
  // API endpoint для smart-chat с поддержкой команды "нужен вектор"
  app.post('/api/smart-chat', async (req, res) => {
    try {
      const { message, sessionId } = req.body;
      
      Logger.info('Получен запрос smart-chat', { message, sessionId });
      
      // Проверяем команду "нужен вектор"
      if (message && message.toLowerCase().includes('нужен вектор')) {
        Logger.ai('Обнаружена команда "нужен вектор"');
        ...
        if (!urlMatch) {
          return res.json({
            success: false,
            error: 'Не найден URL изображения. Используйте: "нужен вектор [URL]"'
          });
        }
        
        const imageUrl = urlMatch[0];
        Logger.info('Найден URL изображения', { imageUrl });
        ...
        try {
          ...
          Logger.success('Изображение загружено', { size: imageBuffer.length });
          
          // Векторизуем через Adobe алгоритм
          const advancedVectorizer = require('../advanced-vectorizer.cjs');
          const startTime = Date.now();
          ...
          Logger.success('Векторизация завершена', {
            duration: duration + 'мс',
            size: sizeKB + 'KB',
            pathCount: (svgContent.match(/<path/g) || []).length
          });
          ...
          const response = {
            success: true,
            message: `✅ Векторизация завершена!\n📄 Размер: ${sizeKB}KB\n🎯 Алгоритм: Adobe Illustrator\n⚡ Время: ${duration}мс`,
            vectorized: true,
          ...
          Logger.error('Ошибка векторизации', vectorError);
          return res.json({
            success: false,
            error: 'Ошибка при векторизации изображения: ' + vectorError.message
          });
        }
      }
      
      // Если не команда векторизации, возвращаем стандартный ответ
      return res.json({
        success: true,
        message: 'Сообщение получено. Для векторизации используйте: "нужен вектор [URL]"'
      });
      
    } catch (error) {
      Logger.error('Ошибка smart-chat API', error);
      res.status(500).json({
        success: false,
